"""
================================================================================
⚡ CHESS AI v26 - IMPROVED POLICY TRAINING + VS STOCKFISH
================================================================================

🔧 IMPROVEMENTS OVER V25:
1. SMARTER AUTO-FREEZE - Uses 3-eval moving average, 8% threshold
2. BUFFER REFRESH - Clears old policy data every 30 iters
3. HIGHER POLICY LR - 1e-8 (10x more than v25, still safe)
4. MORE POLICY TRAINING - Every 3 iters, warmup only 5 iters
5. FASTER CURRICULUM - Skill up at 50% WR
6. CHECKPOINT SAVES - Every 25 iterations
7. BETTER EVAL - 100 games for less variance

Target: Continue from v25_best_sf.pt (33% WR) -> 50%+ WR vs SF skill 0
================================================================================
"""
...
print("=" * 70)
print("⚡ CHESS AI v26 - IMPROVED POLICY TRAINING")
print("=" * 70)
if torch.cuda.is_available():
...
class Config:
    ...
    lr_value: float = 5e-7              # Value head LR (same)
    lr_policy: float = 1e-8             # Policy head LR (10x higher than v25!)
    weight_decay: float = 1e-5
    batches_per_iter: int = 25
    
    # Policy training - IMPROVED
    policy_update_interval: int = 3     # Every 3 iters (was 5)
    policy_warmup: int = 5              # Shorter warmup (was 10)
    wr_drop_threshold: float = 0.08     # 8% tolerance (was 5%)
    min_policy_evals: int = 3           # Need 3 evals before checking drop
    min_win_games_for_policy: int = 5   # Need at least 5 wins
    
    # Buffer management - NEW
    buffer_refresh_interval: int = 30   # Refresh policy buffer every 30 iters
    
    # Stockfish settings
    stockfish_skill: int = 0
    stockfish_depth: int = 1
    skill_up_threshold: float = 0.50    # Lower threshold (was 0.65)
    ...
    eval_games: int = 30
    eval_interval: int = 5
    stockfish_eval_games: int = 100     # Double for less variance (was 50)
    
    # Checkpoint - NEW
    checkpoint_interval: int = 25       # Save every 25 iterations

config = Config()
print(f"\n📋 Configuration (V26 IMPROVED):")
print(f"   Value LR: {config.lr_value}")
print(f"   Policy LR: {config.lr_policy} (10x higher than v25!)")
print(f"   Policy update: every {config.policy_update_interval} iters")
print(f"   Policy warmup: {config.policy_warmup} iters")
print(f"   Auto-freeze: {config.wr_drop_threshold*100:.0f}% drop over {config.min_policy_evals} evals")
print(f"   Buffer refresh: every {config.buffer_refresh_interval} iters")
print(f"   Eval games: {config.stockfish_eval_games}")
...
# ==============================================================================
# Cell 5: Load v25 Best Model
# ==============================================================================

MODEL_PATHS = [
    '/kaggle/input/supervisedmodel/pytorch/default/9/chess_v25_best_sf.pt',  # Primary v25 best
    '/kaggle/input/supervisedmodel/pytorch/default/9/chess_v25_final.pt',
    '/kaggle/input/supervisedmodel/pytorch/default/8/chess_v24_final.pt',    # Fallback to v24
    '/kaggle/working/chess_v25_best_sf.pt',
]
...
def play_game_vs_stockfish(network: ChessNet, sf: Stockfish, config: Config, 
    ...
    for state, player_sign in value_history:
        value = outcome
        weight = 1.0 if outcome != 0 else 0.3
...
def generate_endgame_positions(n_positions: int) -> List[Tuple[np.ndarray, float]]:
    ...
    endgame_types = [
        ('KQ', 'K', 1.0), ('KR', 'K', 1.0), ('KBB', 'K', 1.0),
        ('KBN', 'K', 0.9), ('KP', 'K', 0.8), ('K', 'KQ', -1.0),
        ('K', 'KR', -1.0), ('KR', 'KR', 0.0), ('KQ', 'KQ', 0.0),
        ('KP', 'KP', 0.0), ('KRP', 'KR', 0.5), ('KR', 'KRP', -0.5),
        ('KQP', 'KQ', 0.4), ('KRR', 'K', 1.0), ('K', 'KRR', -1.0),
    ]
...
class PolicyBuffer:
    ...
    def clear(self):
        """Clear buffer to allow fresh data."""
        self.buffer.clear()
...
# ==============================================================================
# Cell 10: Trainer (IMPROVED)
# ==============================================================================

class Trainer:
    """Trainer with improved policy training support."""
    ...
    def __init__(self, network: ChessNet, config: Config, 
        ...
        # Track WR history for moving average (NEW)
        self.wr_history = []
    ...
    def disable_policy_training(self, permanent: bool = False):
        ...
        if permanent:
            ...
            print("⚠️ Policy training PERMANENTLY disabled (consistent WR drop detected)")
        else:
            print("⚠️ Policy training disabled")
    
    def check_should_freeze(self, current_wr: float) -> bool:
        """Check if should freeze policy using moving average (IMPROVED)."""
        self.wr_history.append(current_wr)
        
        # Need at least min_policy_evals to check
        if len(self.wr_history) < self.config.min_policy_evals:
            return False
        
        # Compare current avg (last 2) vs previous avg (before that)
        ...
        if drop >= self.config.wr_drop_threshold:
            print(f"   📉 WR dropped: {older_avg:.1%} -> {recent_avg:.1%} (drop={drop:.1%})")
            return True
...
# ==============================================================================
# Cell 12: Main Training Loop (IMPROVED)
# ==============================================================================
...
def train_v26():
    ...
    print("\n" + "=" * 70)
    print("🚀 STARTING v26 - IMPROVED POLICY TRAINING")
    print("=" * 70)
    ...
    for iteration in range(1, config.rl_iterations + 1):
        ...
        if iteration > 1 and iteration % config.buffer_refresh_interval == 0:
            old_size = len(policy_buffer)
            policy_buffer.clear()
            print(f"🔄 Policy buffer refreshed (was {old_size}, now 0)")
        ...
        if stockfish:
            ...
            print(f"   Value Buffer: {len(buffer)} | Policy Buffer: {len(policy_buffer)}")
        ...
        if len(buffer) >= config.min_buffer_size:
            ...
            history['policy_loss'].append(train_metrics.get('policy_loss', 0))
            history['policy_acc'].append(train_metrics.get('policy_acc', 0))
        ...
        if iteration % config.eval_interval == 0:
            ...
            if stockfish:
                ...
                if wr_sf > best_sf_wr:
                    best_sf_wr = wr_sf
                    torch.save(network.state_dict(), f'{SAVE_DIR}/chess_v26_best_sf.pt')
                    print(f"   ✨ New best vs Stockfish! Saved.")
                
                # IMPROVED: Check for WR drop using moving average
                if trainer.policy_training_enabled and not policy_frozen:
                    if trainer.check_should_freeze(wr_sf):
                        print(f"\n⚠️ Consistent WR drop detected! Freezing policy PERMANENTLY.")
        ...
        if iteration % config.checkpoint_interval == 0:
            torch.save(network.state_dict(), f'{SAVE_DIR}/chess_v26_iter{iteration}.pt')
            print(f"   💾 Checkpoint saved: chess_v26_iter{iteration}.pt")
    ...
    # Final save
    torch.save(network.state_dict(), f'{SAVE_DIR}/chess_v26_final.pt')
    print(f"\n💾 Saved: chess_v26_final.pt")
...
if __name__ == "__main__":
    ...
    if history['policy_loss']:
        axes[1, 1].plot(history['iterations'], history['policy_loss'], 'purple')
        axes[1, 1].set_title('Policy Loss (when trained)')
    ...
    plt.tight_layout()
    plt.savefig(f'{SAVE_DIR}/training_v26.png', dpi=150)
    plt.show()
    
    print("\n🎉 v26 TRAINING COMPLETE!")
    print("=" * 70)
